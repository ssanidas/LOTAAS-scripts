...
class CollectStatsPerBeam(Utilities):
    """                
    ...
    def main(self,argv=None):
        ...
        obsFile    = directory + "/" + "ObservationData.txt"
        beamFile   = directory + "/" + "BeamData.txt"
                ...
                if(len(content)>0):
                    ...
                    for line in content:
                        ...
                        if(statsObs.partOfSameObservation(candidatePath)):
                            ...
                            statsObs.saveAndClearObs(obsFile)
                            statsObs.computeObservationTime(candidatePath)
                            statsObs.update(snr,dm,period)
                            self.candidatesProcessed+=1
                        ...
                        if(statsBeam.partOfSameObservation(candidatePath) and statsBeam.partOfSameBeam(candidatePath) == 1):
                            ...
                            statsBeam.update(snr,dm,period)
                            ...
                            # the data in the stats object, and begin storing the stats again.
                            statsBeam.saveAndClearBeam(beamFile)
                            statsBeam.computeObservationTime(candidatePath)
                            statsBeam.update(snr,dm,period)
    ...
    def outputFileHeaders(self,obsPath,beamPath):
        ...
        header_1 = "Day,Month,Year,Hour,Min,Min SNR,Max SNR,Mean SNR,Median SNR,Q1 SNR,Q3 SNR,IQR SNR,Range SNR,"+\
        "VAR SNR,STDEV SNR,sum SNR,Skew SNR,Kurt SNR,p1 SNR,p1 SNR p-value,p2 SNR,p2 SNR p-value,Count SNR,"   
        
        header_2 = "Min DM,Max DM,Mean DM,Median DM,Q1 DM,Q3 DM,IQR DM,Range DM,"+\
        "VAR DM,STDEV DM,sum DM,Skew DM,Kurt DM,p1 DM,p1 DM p-value,p2 DM,p2 DM p-value,Count DM,"
        
        header_3 = "Min P0,Max P0,Mean P0,Median P0,Q1 P0,Q3 P0,IQR P0,Range P0,"+\
        "VAR P0,STDEV P0,sum P0,Skew P0,Kurt P0,p1 P0, p1 P0 p-value,p2 P0,p2 P0 p-value,Count P0"                                                                       
                                                                                            
        self.appendToFile(obsPath,header_1+header_2+header_3+",Date\n")
        self.appendToFile(beamPath,header_1+header_2+header_3+",Beam,Date\n")                                                                 
                                                                                                                                             
    # ****************************************************************************************************
         
if __name__ == '__main__':
    CollectStatsPerBeam(True).main()