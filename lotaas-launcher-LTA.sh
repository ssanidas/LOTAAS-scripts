...
do
	...
	then
		...
		sbatch $LOTAAS_PL/lotaas_fil_LTA.sh $1 ${sap} ${beam} >job_id_$1.txt
...
#checking the FOLDS directories
for i in SAP0 SAP1 SAP2;do
    for j in `seq 0 73`;do
        if [ ! -d ${outdir}/$1/${i}/${1}_${i}_BEAM${j}/BEAM${j}_sift/FOLDS ];then
        echo "FOLDS directory in ${1}_${i}_BEAM${j} does not exist" >>${outdir}/$1/FOLDS_missing.txt
        fi
    done
done

echo ""
foldsdirs=`find ${outdir}/$1/ -name FOLDS|wc -l`
echo "${foldsdirs} FOLDS directories present"
echo ""
