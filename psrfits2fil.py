...
def translate_header(fits_file):
    ...
    if fits_hdr['TELESCOP'] == "LOFAR":
        # print fits_hdr['OBSFREQ'],subint_hdr['NCHAN'], fits_hdr['OBSBW'], fits_file['SUBINT'].header['CHAN_BW']
	# SC and BWS changed to reduced by half a channel 29/04/14	
        fil_header["fch1"] = fits_hdr['OBSFREQ'] + np.abs(fits_file['SUBINT'].header['CHAN_BW']) * subint_hdr['NCHAN'] / 2.0 - np.abs(fits_file['SUBINT'].header['CHAN_BW']) / 2.0
	print fil_header["fch1"]
    else:
...
def main(fits_fn, outfn, nbits, \
	...
	#subint = subint.astype(np.float) old modification by Daniele 20160613
        np.true_divide(subint,scale_fac,out=subint,casting='unsafe') #latest modification by Daniele 20160613
        #subint /= scale_fac original
