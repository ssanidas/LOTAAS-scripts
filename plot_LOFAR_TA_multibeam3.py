...
def wmom(arrin, weights_in, inputmean=None, calcerr=False, sdev=False):
   ...
   """
    ...
    arr = np.nan_to_num(np.array(arrin, ndmin=1, copy=False))
    
    # Weights is forced to be type double. All resulting calculations
    # will also be double
    weights = np.nan_to_num(np.array(weights_in, ndmin=1, dtype='f8', copy=False))
...
if __name__ == '__main__':
	...
	for line in file.readlines():
		...
		if line.find("Observation.Beam[%s].subbandList" % (options.sap)) >= 0:
	...
	for line in file.readlines():
		psr=line.split("_")[-1].split(" S/N")[0]
		if psr != options.target: continue
		beam = int(line.split("obs=")[1].split("_SAP%s_BEAM" % (options.sap))[1].split("_")[0])
#		chisq = float(line.split("chi-sq=")[1])
		chisq = float(line.split("S/N=")[1].split(" ")[0])
...
#	    print i, val, dsq[i], npoints, options.nrings
	...
	xmean,xerr = np.nan_to_num(wmom(x, weights, inputmean=None, calcerr=True, sdev=False))
	ymean,yerr = np.nan_to_num(wmom(y, weights, inputmean=None, calcerr=True, sdev=False))
#	print xmean, ymean
#	print rarad, decrad
...
#	print "DEC_err: %f arcmin" % (yerr*60)
        ...
        print "Best-fit position of PSR %s: (RAJ, DECJ) = ( %s +/- %s , %s +/- %s )" % (options.target, str_f(src._ra), str_f(err._ra), str_f(src._dec), str_f(err._dec))
